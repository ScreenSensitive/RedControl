...
# Launch through a small wrapper rather than putting the command in Exec
# directly: Desktop Entry treats ; > & and quotes as reserved, so a redirect in
# Exec fails validation and some launchers refuse it outright. The wrapper also
# captures stderr — with Terminal=false a startup failure (missing tkinter, a
# traceback) otherwise gives no window and no message, which looks exactly like
# a shortcut that does nothing.
BINDIR="$HOME/.local/bin"
mkdir -p "$BINDIR"
cat > "$BINDIR/redcontrol" <<EOF
#!/bin/sh
# Generated by RedControl's install.sh
mkdir -p "\$HOME/.cache"
cd "$HERE" || exit 1
exec python3 "$HERE/redcontrol.py" "\$@" >>"\$HOME/.cache/redcontrol-launch.log" 2>&1
EOF
chmod +x "$BINDIR/redcontrol"
