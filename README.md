...
- **Force Link Depth** writes the encoder's depth field directly (`DP_COMPONENT_DEPTH` on DP, `HDMI_DEEP_COLOR_DEPTH`/`ENABLE` on HDMI). Every force goes through a Keep/Revert dialog that auto-reverts in 10 s if unanswered. A depth you keep is remembered, and **Auto-Reapply** (Settings menu) restores it whenever the driver resets the register on a modeset or monitor sleep/wake.
